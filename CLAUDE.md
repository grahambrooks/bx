...
- **Windows AppContainer specifics.** `appcontainer::build_plan` is pure (no syscalls), unit-tested on every host like the other two generators; the Win32 *application* lives in `exec.rs`'s `#[cfg(windows)] mod win` and is compile-tested only on the Windows CI runner (it cannot link on macOS/Linux because reqwest→ring needs a Windows C toolchain). AppContainer's filesystem model is ACL-based, not a launch-time allow-list, so `win` mutates DACLs on the policy paths (grant the package SID, **revert via `GrantGuard` on scope exit**). Two things about that revert, both learned the hard way from CI:

  - **Revert by revoking the trustee, never by restoring a snapshot.** `GrantGuard` re-reads the current DACL and drops every ACE belonging to the package SID (`SetEntriesInAclW` + `REVOKE_ACCESS`). Restoring a DACL captured before the grant *cannot* be faithful: `GetNamedSecurityInfoW` reports inherited ACEs with `INHERITED_ACE` set and `SetNamedSecurityInfoW` will not re-establish those as explicit entries, so the round trip rewrites unrelated ACEs. Revoking touches only what bx added.
  - **Reassert the protection bit on every DACL write.** `SetNamedSecurityInfoW` does not carry `SE_DACL_PROTECTED` across, so `grant_path` reads it with `GetSecurityDescriptorControl` and passes `PROTECTED_`/`UNPROTECTED_DACL_SECURITY_INFORMATION`. Without it a *protected* DACL comes back unprotected and starts inheriting the parent's ACEs — a genuine widening. It fails closed if the control bits are unreadable.

  What is *not* a bug, and what `windows_sandbox_reverts_dacls_on_exit` must therefore tolerate: writing any DACL on an unprotected object makes Windows re-run automatic inheritance, so parent-derived ACEs that were unflagged copies (which is how `CreateDirectory` leaves them) come back marked `(I)`. Same principals, same rights, nothing bx can prevent. The test strips that marker and asserts the two things that do matter — no `S-1-15-2-` ACE survives, and the principal/rights set is unchanged. Note the second assertion still catches a protection loss, since that pulls in principals the object did not have. This is the only host-state mutation in any backend. The `strict` profile sets `Policy.lpac` (Less-Privileged AppContainer); looser profiles leave it off so binaries with dependencies unreadable by `ALL RESTRICTED APPLICATION PACKAGES` still load. Win32 constants in `win` are defined locally (stable ABI) to minimise dependence on exact `windows-sys` symbol paths.
